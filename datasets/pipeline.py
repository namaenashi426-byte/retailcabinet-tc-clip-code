...
@PIPELINES.register_module()
class ColorJitter:
    def __init__(self, p=0.8, p_gray=0.2, brightness=0.4, contrast=0.4,saturation=0.2, hue=0.1, clip_consistent=True):
        self.p = p
        self.p_gray = p_gray
        self.clip_consistent = clip_consistent
        self.worker = torchvision.transforms.ColorJitter(brightness=brightness, contrast=contrast, saturation=saturation, hue=hue)

    def _apply_clip_jitter(self, imgs):
        params = self.worker.get_params(
            self.worker.brightness,
            self.worker.contrast,
            self.worker.saturation,
            self.worker.hue)
        ...
        return [np.asarray(transform(Image.fromarray(img))) for img in imgs]

    def __call__(self, results):
        imgs = results['imgs']
        v = random.random()
        if v < self.p:
            if self.clip_consistent:
                imgs = self._apply_clip_jitter(imgs)
            else:
    ...
    def __repr__(self):
        repr_str = (f'{self.__class__.__name__}'
                    f'(clip_consistent={self.clip_consistent})')
        return repr_str
...
@PIPELINES.register_module()
class MotionAwareSampleFrames:
    """Segment-wise motion-aware frame sampling.

    This sampler keeps global temporal coverage while adding low-cost
    frame-difference peaks. It is intended to run after ``DecordInit`` and
    before ``DecordDecode`` because it reads a small set of candidate frames
    from ``results['video_reader']``.
    """
    ...
    def __init__(self,
        ...
        self.candidate_frames = max(1, int(candidate_frames))
        self.motion_size = max(1, int(motion_size))
        self.motion_ratio = float(np.clip(motion_ratio, 0.0, 1.0))
        self.keep_boundary = keep_boundary
        self.num_boundary = max(0, int(num_boundary))
        self.pair_delta = max(0, int(pair_delta))
        self.pair_count = None if pair_count is None else max(1, int(pair_count))
        self.out_of_bound_opt = out_of_bound_opt
        self.multiview = max(1, int(multiview))

        valid_modes = ['hybrid', 'motion', 'motion_only', 'uniform',
                       'transition_pair']
    ...
    def target_count(self):
        return self.clip_len * self.num_clips

    def _uniform_indices(self, total_frames, count, view_idx=0):
        count = int(count)
        if count <= 0:
        ...
        edges = np.linspace(0, total_frames, count + 1)
        indices = []
        if self.test_mode:
            frac = 0.5
            if self.multiview > 1:
                frac = (view_idx + 0.5) / self.multiview
            for start, end in zip(edges[:-1], edges[1:]):
                pos = start + (end - start) * frac
                indices.append(int(np.clip(np.floor(pos), 0, total_frames - 1)))
        else:
            for start, end in zip(edges[:-1], edges[1:]):
                low = int(np.floor(start))
                high = int(np.ceil(end))
                low = int(np.clip(low, 0, total_frames - 1))
                high = int(np.clip(max(high, low + 1), 1, total_frames))
                indices.append(int(np.random.randint(low, high)))

        return np.array(indices, dtype=np.int64)

    def _boundary_indices(self, total_frames, count):
        if count <= 0:
            return np.empty((0,), dtype=np.int64)
        count = min(count, self.target_count)
        if total_frames <= 1:
            return np.zeros((count,), dtype=np.int64)

        left_count = count // 2
        right_count = count - left_count
        left = np.arange(left_count, dtype=np.int64)
        right_start = max(total_frames - right_count, 0)
        right = np.arange(right_start, total_frames, dtype=np.int64)
        return np.concatenate([left, right])

    def _to_small_gray(self, frame):
        ...
        if frame.ndim == 3 and frame.shape[-1] >= 3:
            ...
            gray = frame.squeeze().astype(np.float32, copy=False)

        height, width = gray.shape[:2]
        y_idx = np.linspace(0, height - 1, self.motion_size).astype(np.int64)
        x_idx = np.linspace(0, width - 1, self.motion_size).astype(np.int64)
        return gray[y_idx][:, x_idx]

    def _read_candidate_frames(self, container, total_frames):
        candidate_count = min(self.candidate_frames, total_frames)
        candidate_inds = np.linspace(
            0, total_frames - 1, candidate_count).round().astype(np.int64)
        candidate_inds = np.unique(candidate_inds)

        try:
            frames = container.get_batch(candidate_inds.tolist()).asnumpy()
        except Exception:
            frames = np.stack([
                container[int(idx)].asnumpy() for idx in candidate_inds
            ], axis=0)

        return candidate_inds, frames

    def _motion_scores(self, container, total_frames):
        candidate_inds, frames = self._read_candidate_frames(
            container, total_frames)
        if len(candidate_inds) == 0:
            return candidate_inds, np.empty((0,), dtype=np.float32)

        grays = [self._to_small_gray(frame) for frame in frames]
        scores = np.zeros((len(grays),), dtype=np.float32)
        for idx in range(1, len(grays)):
            scores[idx] = np.mean(np.abs(grays[idx] - grays[idx - 1]))
        return candidate_inds, scores

    def _motion_peak_indices(self, total_frames, candidate_inds, scores,
                             count, view_idx=0):
        count = int(count)
        if count <= 0:
            return np.empty((0,), dtype=np.int64)
        if len(candidate_inds) == 0:
            return self._uniform_indices(total_frames, count, view_idx)

        fallback = self._uniform_indices(total_frames, count, view_idx)
        edges = np.linspace(0, total_frames, count + 1)
        indices = []
        for seg_idx, (start, end) in enumerate(zip(edges[:-1], edges[1:])):
            if seg_idx == count - 1:
                mask = (candidate_inds >= start) & (candidate_inds <= end)
            else:
                mask = (candidate_inds >= start) & (candidate_inds < end)

            local = np.where(mask)[0]
            if len(local) == 0 or np.max(scores[local]) <= 0:
                indices.append(int(fallback[seg_idx]))
                continue

            order = local[np.argsort(scores[local])[::-1]]
            rank = min(view_idx, len(order) - 1) if self.test_mode else 0
            indices.append(int(candidate_inds[order[rank]]))

        return np.array(indices, dtype=np.int64)

    def _transition_pair_indices(self, total_frames, candidate_inds, scores,
                                 count, view_idx=0):
        count = int(count)
        if count <= 0:
            return np.empty((0,), dtype=np.int64)

        max_pairs = max(1, int(np.ceil(count / 2.0)))
        pair_count = self.pair_count if self.pair_count is not None else max_pairs
        pair_count = min(pair_count, max_pairs)

        peaks = self._motion_peak_indices(
            total_frames, candidate_inds, scores, pair_count, view_idx)
        pairs = []
        for peak in peaks:
            pairs.extend([int(peak) - self.pair_delta,
                          int(peak) + self.pair_delta])
        return np.array(pairs, dtype=np.int64)

    def _finalize_indices(self, indices, total_frames, target_count, view_idx):
        clipped = [
            int(np.clip(idx, 0, max(total_frames - 1, 0))) for idx in indices
        ]
        final = []
        seen = set()
        for idx in sorted(clipped):
            if idx not in seen:
                final.append(idx)
                seen.add(idx)

        fill_pools = [
            self._uniform_indices(total_frames, target_count, view_idx),
            np.linspace(0, max(total_frames - 1, 0), target_count).round()
            .astype(np.int64)
        ]
        for pool in fill_pools:
            for idx in pool:
                if len(final) >= target_count:
                    break
                idx = int(np.clip(idx, 0, max(total_frames - 1, 0)))
                if idx not in seen:
                    final.append(idx)
                    seen.add(idx)
            if len(final) >= target_count:
        ...
        repeat_pool = self._uniform_indices(total_frames, target_count, view_idx)
        cursor = 0
        while len(final) < target_count:
            final.append(int(repeat_pool[cursor % len(repeat_pool)]))
            cursor += 1

        final = np.array(sorted(final), dtype=np.int64)
        if len(final) > target_count:
            keep = np.linspace(0, len(final) - 1, target_count).round()
            final = final[keep.astype(np.int64)]
        return final

    def _sample_once(self, total_frames, candidate_inds, scores, view_idx):
        target_count = self.target_count
        boundary = np.empty((0,), dtype=np.int64)
        if self.keep_boundary:
            boundary = self._boundary_indices(total_frames, self.num_boundary)

        remaining = max(target_count - len(np.unique(boundary)), 0)
        if self.mode == 'uniform':
            sampled = self._uniform_indices(total_frames, remaining, view_idx)
        elif self.mode in ['motion', 'motion_only']:
            sampled = self._motion_peak_indices(
                total_frames, candidate_inds, scores, remaining, view_idx)
        elif self.mode == 'transition_pair':
            sampled = self._transition_pair_indices(
                total_frames, candidate_inds, scores, remaining, view_idx)
        else:
            motion_count = int(round(remaining * self.motion_ratio))
            if remaining > 1 and self.motion_ratio > 0:
                motion_count = max(1, motion_count)
            motion_count = min(motion_count, remaining)
            uniform_count = remaining - motion_count
            uniform = self._uniform_indices(
                total_frames, uniform_count, view_idx)
            motion = self._motion_peak_indices(
                total_frames, candidate_inds, scores, motion_count, view_idx)
            sampled = np.concatenate([uniform, motion])

        selected = np.concatenate([boundary, sampled])
        return self._finalize_indices(
            selected, total_frames, target_count, view_idx)
    ...
    def __call__(self, results):
        ...
        if self.mode == 'uniform':
            candidate_inds = np.empty((0,), dtype=np.int64)
            scores = np.empty((0,), dtype=np.float32)
        else:
            candidate_inds, scores = self._motion_scores(
                container, total_frames)

        view_indices = [
            self._sample_once(total_frames, candidate_inds, scores, view_idx)
            for view_idx in range(self.multiview)
        ]
        frame_inds = np.concatenate(view_indices)
    ...
    def __repr__(self):
        repr_str = (f'{self.__class__.__name__}('
                    ...
                    f'test_mode={self.test_mode}, '
                    f'mode={self.mode}, '
                    f'candidate_frames={self.candidate_frames}, '
                    f'motion_size={self.motion_size}, '
                    f'motion_ratio={self.motion_ratio}, '
                    f'keep_boundary={self.keep_boundary}, '
                    f'num_boundary={self.num_boundary}, '
                    f'pair_delta={self.pair_delta}, '
                    f'pair_count={self.pair_count}, '
                    f'multiview={self.multiview})')
        return repr_str
