...
def _use_motion_sampling(config):
    return config.get("sample_type", "uniform") in [
        "motion", "motion_aware", "smhs", "transition_pair"
    ]


def _disable_crop(config):
    return bool(_cfg_get(config, "disable_crop", False))


def _full_frame_resize(input_size):
    return dict(type='Resize', scale=(input_size, input_size), keep_ratio=False)


def _build_sample_cfg(config, test_mode=False, multiview=1):
    if _use_motion_sampling(config):
        motion_cfg = config.get("motion_sampler", {})
        motion_mode = _cfg_get(motion_cfg, "mode", "hybrid")
        if config.get("sample_type", "uniform") == "transition_pair" and motion_mode == "hybrid":
            motion_mode = "transition_pair"
        return dict(
            type='MotionAwareSampleFrames',
            clip_len=1,
            frame_interval=1,
            num_clips=config.num_frames,
            test_mode=test_mode,
            mode=motion_mode,
            candidate_frames=_cfg_get(motion_cfg, "candidate_frames", 48),
            motion_size=_cfg_get(motion_cfg, "motion_size", 64),
            motion_ratio=_cfg_get(motion_cfg, "motion_ratio", 0.5),
            keep_boundary=_cfg_get(motion_cfg, "keep_boundary", False),
            num_boundary=_cfg_get(motion_cfg, "num_boundary", 2),
            pair_delta=_cfg_get(motion_cfg, "pair_delta", 2),
            pair_count=_cfg_get(motion_cfg, "pair_count", None),
            out_of_bound_opt=_cfg_get(motion_cfg, "out_of_bound_opt", "repeat_last"),
            multiview=multiview,
        )
    ...
    if test_mode:
        sample_cfg["test_mode"] = True
    if multiview > 1:
        sample_cfg["multiview"] = multiview
    return sample_cfg
...
def build_train_dataloader(logger, config):
    ...
    disable_crop = _disable_crop(config)
    if disable_crop:
        logger.info(f"Train crop disabled: resizing full frames to {config.input_size}x{config.input_size}")
        spatial_pipeline = [_full_frame_resize(config.input_size)]
    else:
        spatial_pipeline = [
    ...
    train_pipeline = [
        ...
        _build_sample_cfg(config, test_mode=False),   # TSN uniform or SMHS motion-aware sampling
...
def build_val_dataloader(logger, config, target_data_config):
    ...
    disable_crop = _disable_crop(config)
    if disable_crop:
        if config.num_crop != 1:
            logger.info(f"Validation crop disabled: forcing num_crop from {config.num_crop} to 1")
            config.num_crop = 1
        logger.info(f"Validation crop disabled: resizing full frames to {config.input_size}x{config.input_size}")
        spatial_pipeline = [_full_frame_resize(config.input_size)]
    else:
        spatial_pipeline = [
            dict(type='Resize', scale=(-1, scale_resize)),
    ...
    val_pipeline = [
        dict(type='DecordInit'),
        _build_sample_cfg(config, test_mode=True),
        dict(type='DecordDecode'),
        *spatial_pipeline,
        dict(type='Normalize', **img_norm_cfg),
        dict(type='FormatShape', input_format='NCHW'),
    ...
    if not disable_crop and config.num_crop == 3:
    ...
    if config.num_clip > 1:
        if _use_motion_sampling(config):
            val_pipeline[1] = _build_sample_cfg(config, test_mode=True, multiview=config.num_clip)
        else:
